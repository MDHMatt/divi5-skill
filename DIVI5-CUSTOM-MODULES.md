...
> Skill files: BASE · LAYOUT · STYLING · MODULES-CONTENT · MODULES-INTERACTIVE · MODULES-MEDIA · MODULES-DATA · WORDPRESS · PATTERNS · CUSTOM-MODULES (this)

> **✅ RENDER-CONFIRMED on Divi 5.9.0 (2026-07-21).** This contract was documented
> but never built against real code until a real 3-module pack (an FAQ
> parent/child pair emitting schema.org FAQPage JSON-LD) validated it end-to-end:
> both block types register, a generated layout imports via the portability helper,
> parent/child nesting survives, and the modules render on the front end.
...
```
<!-- wp:vendor/my-module {"builderVersion":"5.7.0","title":{...}} /-->
```

`"builderVersion":"5.7.0"` is **required** on custom modules, just as on all built-in modules.
...
`$elements->style(['attrName' => 'module'])`. A parent FAQ module's `FaqModule.php`
render is a worked example.
...
This maps directly to the encoding rules in DIVI5-BASE Rule 9 and DIVI5-MODULES-CONTENT.
...
```
<!-- wp:vendor/my-parent {"builderVersion":"5.7.0"} -->
<!-- wp:vendor/my-child {"builderVersion":"5.7.0"} /-->
<!-- wp:vendor/my-child {"builderVersion":"5.7.0"} /-->
<!-- /wp:vendor/my-parent -->
```
...
pattern an FAQ module uses to emit FAQPage JSON-LD.
...
*DIVI5 Custom Modules Skill — V0.6.0 | Builder Version 5.9.0 | Created by Shashank Gupta @ divilove.com*
*Render-confirmed + corrected on Divi 5.9.0 (2026-07-21) against a real 3-module pack — see the status note at the top.*
